...
function ProductModal({
  ...
  if (
    ...
    categoryOptions.unshift({
      ...
      Nombre: `Categoría actual (${currentCategoryId})`,
    });
  }
  ...
  const handleFilesSelected = async (event) => {
    ...
    try {
      ...
      showErrorToast(error.message || 'No se pudieron cargar las imágenes.');
    } finally {
  ...
  const handleFormSubmit = handleSubmit((values) => {
    try {
      ...
      showErrorToast(error.message || 'Revisa la información del producto.');
    }
  });
          ...
          <section className="space-y-6">
            <div className="rounded-[28px] border border-brand-mist/70 bg-white/70 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-brand-forest">Información general</p>
                  <p className="text-sm text-brand-moss">
                    Define el producto, su categoría y la forma en que se venderá.
                  </p>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  >
                    <option value="">
                      {hasCategoryOptions ? 'Selecciona una categoría' : 'No hay categorías disponibles'}
                    </option>
                  ...
                  <label className="flex items-center justify-between gap-4" htmlFor="producto-usa-tallas">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-brand-moss">
                        Activa esta opción si el precio y stock se controlan por talla.
                      </p>
                    </div>
                    <input
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    {...register('Descripcion', {
                      maxLength: {
                        value: 800,
                        message: 'La descripción no debe superar 800 caracteres.',
                      },
                    })}
                  />
                  {errors.Descripcion ? (
            ...
            <div className="rounded-[28px] border border-brand-mist/70 bg-white/70 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-brand-forest">Imágenes del producto</p>
                  <p className="text-sm text-brand-moss">
              ...
              {imageFields.length === 0 ? (
                <div className="mt-5 rounded-[24px] border border-dashed border-brand-mist bg-brand-cream/50 p-5 text-sm text-brand-moss">
                  Todavía no hay imágenes agregadas para este producto.
                </div>
              ) : (
              ...
              {!usaTallas ? (
                    ...
                    <input
                      ...
                      {...register('PrecioMenor', {
                        validate: (value) => (
                          usaTallas
                          || String(value ?? '').trim() !== ''
                          || 'El precio menor es obligatorio cuando no usas tallas.'
                        ),
                      })}
                    />
                    {errors.PrecioMenor ? (
                    ...
                    <input
                      ...
                      {...register('PrecioMayor', {
                        validate: (value) => (
                          usaTallas
                          || String(value ?? '').trim() !== ''
                          || 'El precio mayor es obligatorio cuando no usas tallas.'
                        ),
                      })}
                    />
                    {errors.PrecioMayor ? (
                ...
                <div className="mt-5 rounded-[24px] border border-brand-mist/60 bg-brand-cream/60 p-4">
                  ...
                  <p className="mt-2 text-sm text-brand-moss">
                    El backend calculará `PrecioMenor` y `PrecioMayor` usando las tallas activas.
                  </p>
            ...
            <div className="rounded-[28px] border border-brand-mist/70 bg-white/70 p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-brand-moss">
                    Registra stock y precios por talla cuando `UsaTallas` esté activo.
                  </p>
                </div>
              </div>
              ...
              {usaTallas ? (
                sizeFields.length === 0 ? (
                  <div className="mt-5 rounded-[24px] border border-dashed border-brand-mist bg-brand-cream/50 p-5 text-sm text-brand-moss">
                    Todavía no hay tallas cargadas para este producto.
                  </div>
                ) : (
                  <div className="mt-5 space-y-4">
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-brand-mist">
                    {watch('CategoriaID')
                      ? `${categoryOptions.find((category) => String(category.CategoriaID) === String(watch('CategoriaID')))?.Nombre || 'Categoría seleccionada'}`
                ...
                <div className="rounded-[24px] border border-white/10 bg-white/5 px-4 py-3">
                  ...
                  <p className="mt-2 text-sm font-semibold">
                    {usaTallas
                      ...
                      : `${imageFields.length} imágenes registradas`}
                  </p>
                </div>
              </div>
                ...
                <label className="mt-5 flex items-center justify-between gap-4 rounded-[24px] border border-white/10 bg-white/5 px-4 py-4" htmlFor="producto-estado">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-brand-mist">
                      Si desmarcas esta opción y guardas, el producto se desactivará lógicamente.
                    </p>
                  </div>
                  <input
